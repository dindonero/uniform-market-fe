...
const ClaimBox: React.FC = () => {
  ...
  } = useReadContract(chainId === defaultChain.id ? {
  ...
  const sendNoClaimableBalanceNotification = () => {
    toast({
      title: "No Claimable Amount",
      description: "You don't have any amount to be claimed",
      status: "error",
      duration: 9000,
      isClosable: true,
    });
  ...
  return (
    <Box className="flex justify-center items-center">
      <Box className="flex flex-col px-7 py-9 font-medium bg-white rounded-xl shadow-lg max-w-[526px] max-md:px-5">
        ...
        {isConnected && chainId && defaultChain.id === chainId && isBalanceLoading && (
          <Box className="flex justify-center items-center py-5">
            <Spinner color="blue.500" />
          </Box>
        )}

        {isConnected && chainId && defaultChain.id === chainId && balanceData?.toString() && (
        ...
        {(isBalanceLoading || isWritePending || isConfirming) && (
            <Button
                disabled
        ...
        {isConnected && chainId && defaultChain.id === chainId && !isWritePending && (
          <Button
            onClick={() => balanceData?.toString() === "0" ? sendNoClaimableBalanceNotification() : handleClaim()}
            className="justify-center items-center px-8 py-4 mt-10 text-base leading-4 text-center text-white bg-blue-600 rounded-lg border border-blue-600 border-solid max-md:px-5 max-md:max-w-full"
          >
            Claim
          </Button>
