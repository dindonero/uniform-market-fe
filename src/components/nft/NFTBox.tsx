...
const NFTBox: React.FC = () => {
  ...
  // Mint NFT
  const { writeContract: mintNFT, isPending: isMinting, isSuccess: isMintSuccess } = useWriteContract();
  ...
  const fetchNFTData = useCallback(async () => {
    ...
    try {
      ...
      toast({
        ...
        description: "Unable to fetch NFT metadata. Please try again.",
        status: "error",
        duration: 5000,
        isClosable: true,
      });
  ...
  useEffect(() => {
    if (isMintSuccess) {
      toast({
        title: "NFT Minted!",
        description: "Your new NFT has been minted successfully.",
        status: "success",
        duration: 5000,
        isClosable: true,
      });
  ...
  }, [isMintSuccess, refetchAll, toast]);
